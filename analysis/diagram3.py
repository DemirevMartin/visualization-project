...
def create_splom_view(df):
    ...
    for var in splom_vars:
        if var in splom_data.columns:
            dimensions.append(dict(label=var_labels.get(var, var), values=splom_data[var]))

    service_colors = {'emergency': 0, 'surgery': 1, 'general_medicine': 2, 'ICU': 3}
    splom_data['service_id'] = splom_data['service'].map(service_colors)

    fig_splom = go.Figure(data=go.Parcoords(
        line=dict(
            color=splom_data['service_id'],
            colorscale=[
                [0.0, '#1f77b4'], # Emergency
                [0.33, '#ff7f0e'], # Surgery
                [0.66, '#2ca02c'], # General Medicine
                [1.0, '#d62728']   # ICU
            ],
            showscale=True,
            cmin=0,
            ...
            colorbar=dict(
                ...
                ticktext=['Emergency', 'Surgery', 'General Medicine', 'ICU'],
                len=0.7,
                thickness=15
            )
        ),
    ...
    fig_splom.update_layout(
        title="<b>Scatter Plot Matrix (SPLOM) of Staff Presence vs Patient Outcomes</b>",
        height=600,
